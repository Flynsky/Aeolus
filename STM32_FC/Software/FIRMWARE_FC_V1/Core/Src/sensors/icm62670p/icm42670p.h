...
typedef enum icm_userbank_register
{
    ...
    ACCEL_CONFIG1 = 36, // 6:4 ACEL Averaging
                        // 2:0 ACELL Lowpass
    FIFO_CONFIG1 = 40,  // 1 FIFO_MODE  0:stream to fifo/1:stop on full
                        // 0 FIFO_BYPASS 0:not bypassed
...
{
    TMST_CONFIG1 = 0,
    FIFO_CONFIG5 = 1, // configs fifo content
    FIFO_CONFIG6 = 2,
    FSYNC_CONFIG = 3,
    INT_CONFIG0 = 4,
    INT_CONFIG1 = 5,
    ...
    FDR_CONFIG = 102 // 3:0 FIFO packet rate decimation factor. Sets the number of discarded FIFO
